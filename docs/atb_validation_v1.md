...
**Code commit:** `ebd9fefb` (harness) + `fda6aea0` (model paths) + `19fad378` (loader fix)
...
| V6  | correct_bank dominates all negative controls | ❌ FAIL (metric artifact) |
...
**Implication:** A repeat of Exp 6 using `bank_key_mode="post_rope"` (which *does*
show positive margin in Exp 1) is needed to properly validate negative controls.
This is flagged as a future experiment (`atb_validation_v2/exp6b`).
...
├── finalize.py         # Aggregation + report generator
...
Run: `python3 experiments/atb_validation_v1/finalize.py --exp-root <results_archive> --out experiments/atb_validation_v1/final_report`*

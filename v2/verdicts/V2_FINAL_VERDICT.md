...
**Overall Stance**: **Substantially falsified in its original framing**; see §1b (Headline finding, revised). The mechanism is real but its interpretation has changed. The K-projector + non-empty bank produces a reliable NLL drop, but **eleven** independent wave-3 / wave-5 / wave-6 / wave-7 / wave-8 / wave-9 / wave-10 falsifiers now converge on the "adapter, not memory" reading. The most decisive single test (e16-forgetting, wave-10) shows that after training the projector with set_A in the bank, **completely evicting set_A and replacing it with a different set_B preserves 96.5 % of the improvement on set_A queries** (Δ=4.99 → 4.81), and set_B queries — items never seen during training — get exactly the same boost (Δ=4.82). The bank substrate is what the projector exploits; content is irrelevant.
...
10. **e16 forgetting curve (wave-10)** — Train projector with set_A in bank → Δ_A=**4.99**. Evict set_A entirely via ring-buffer overwrite with set_B → Δ_A_after_evict=**4.81** (loses only 0.17 nat = 3.5 %). Eval set_B (items never seen during training) → Δ_B=**4.82** (identical to set_A). Empty bank → Δ=0.00. **The bank's contents are functionally irrelevant; the projector exploits the bank substrate, not its semantic payload.** This is the cleanest retrieval-vs-adapter prediction-divergent probe in the suite, and it lands squarely on adapter.
...
| **e16-fgt** | Forgetting curve (ring-buffer eviction, seed 0) | Δ_A_initial=**4.99** (train+eval with set_A in bank) → evict set_A by overwriting with set_B → Δ_A_after_evict=**4.81** (drops only 0.17 nat!) — Δ_B=**4.82** on items never seen during training — Δ_A_zero=0.00 (empty bank) | ❌ **The single most decisive content-blindness test.** Erasing the exact set the projector was trained on has near-zero impact (0.17/4.99 ≈ 3% degradation), and an entirely *different* set in the bank gives the same boost as the training set. The bank substrate is what matters; content is irrelevant. |

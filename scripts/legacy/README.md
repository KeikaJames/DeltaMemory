...
|---|---|---|
| `run_clean_demo.py` | Early Mneme attention-injection demo (mock-Gemma default). | _none_ (developer demo) |
...
| `run_stage9_baselines.py` | Stage 9-C head-to-head baselines vs Mneme. | `reports/experiments/stage9*`, `stage10C_*` |
...
| `run_stage11_conv.py` | Stage 11D conversational memory benchmarks for Mneme. | `reports/experiments/stage11_grand_evaluation` |
...
| `run_stage13b_robust.py` | Stage 13B robustness benchmarks on the AttentionNative Mneme bank. | `reports/cleanroom/stage13b_robust`, `stage13b_smoke` |
...
| `run_stage13f_interactive.py` | Stage 13F interactive transcripts with Mneme injection. | `reports/cleanroom/stage13f_interactive` |

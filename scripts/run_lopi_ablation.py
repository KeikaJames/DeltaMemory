...
def main():
    ...
    # Pre-compute base seq-NLL for the neutral set (no bank, no LOPI).
    base_nlls = [seq_nll(model, tok, p, args.device) for p in neutrals]

    cells = []
    for variant_id, cfg_kwargs in VARIANTS.items():
        cfg = LOPIConfig(**cfg_kwargs)
        # Per-variant fresh state.
        ...
        for f in facts:
            ...
            bank = _write_fact(model, tok, wp, n_layers, args.device)
            state.reset()  # fresh causal cache per fact (independent prompts)
            # Baseline (no bank) one-token logp for lift computation.
            with torch.no_grad():
                base_logits = model(**tok(rp, return_tensors="pt").to(args.device)).logits[0, -1].float()
            base_lp = torch.nn.functional.log_softmax(base_logits, dim=-1)[target_id].item()

            inj_logits = _read_with_bank_lopi(
                model, tok, rp, bank, args.alpha, n_layers, args.device,
        ...
        # Drift on neutral prompts.
        neutral_bank = _write_fact(
            model, tok,
            "Fact: The Sun is a star at the centre of the Solar System.",
            n_layers, args.device,
        )
        state.reset()  # fresh causal cache for neutral pass
        inj_nlls = []
        for p in neutrals:
            inj_nlls.append(seq_nll_with_lopi(
                model, tok, p, neutral_bank, args.alpha, n_layers, args.device,
                cfg, state,
            ))
            state.reset()  # reset between independent prompts

        drift = sum(b - i for b, i in zip(inj_nlls, base_nlls)) / len(neutrals)
        # drift sign: positive = injection makes neutral text *worse*.
        drift = -drift  # flip to match mHC3 convention (inj - base)
        ...
        cells.append(cell)
        print(f"[{variant_id}] lift={cell['mean_lift']:+.4f}  drift={cell['mean_drift']:+.4f}")

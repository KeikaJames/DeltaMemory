...
class MhcGPT2Model(MhcGPT2PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # behaviour locally: when no per-head mask is supplied we just
        # broadcast ``None`` across layers, which the downstream attention
        # blocks already treat as identity.
...
class MhcGPT2LMHeadModel(MhcGPT2PreTrainedModel, GenerationMixin):
    ...
    # NOTE (vendor mod): upstream uses ``["lm_head.weight"]`` (list), but
    # transformers >=5 expects a dict {tied_key: source_key}. Aligning to the
    # current GPT2LMHeadModel mapping keeps the conversion regression test
    # green on transformers 5.x without altering algorithmic behaviour.
    ...
    def tie_weights(self, *args, **kwargs):
        ...
        # NOTE (vendor mod): forward extra positional/keyword args (e.g.
        # ``recompute_mapping``) introduced in transformers >=5 so the
        # post_init() call inside ``__init__`` keeps working.

...
5 variants on identical CF prompts:
  * correct_bank
  * shuffled_bank (fact↔target permuted)
...
def make_variants(alpha: float = 1.0) -> list[Variant]:
    base = dict(method="anb", alpha=alpha, bank_key_mode="pre_rope",
                value_scale_mode="auto_rms_cap")
    return [
        Variant(name="correct_bank", **base,
                description="Production ATB; reference."),
...
def main() -> int:
    ...
    write_manifest(
        out_dir,
        experiment="exp6_negative_controls",
        repo_root=ROOT,
        ...
        extra={
            "perturbation_protocol": ("Random K/V draws use "
    ...
    summary = aggregate(res, experiment="exp6_negative_controls",

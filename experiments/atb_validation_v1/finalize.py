...
def analyze_exp7(run_dir: Path) -> dict:
    return _analyze_neg_controls(run_dir)
...
def _write_neg_controls_bar(analysis: dict, path: Path, title: str) -> None:
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
...
def write_exp7_plot(analysis: dict, path: Path) -> None:
    bank_size = analysis.get("bank_size", "?")
    dom = analysis.get("verdict_correct_dominates", False)
    _write_neg_controls_bar(
        analysis, path,
        f"Exp 7: Non-Gemma Pre-RoPE Negative Controls (bank_size={bank_size})\n"
        f"Qwen3-4B-Instruct-2507 | correct_bank dominates = {dom}",
    )


# ---------------------------------------------------------------------------
# Per-run README writers

def _write_run_readme(run_dir: Path, exp_name: str, summary_md: str) -> None:
    """Write a README.md into the run directory."""
    path = run_dir / "README.md"
    path.write_text(summary_md)


def _exp1_readme(a: dict, run_dir: Path) -> None:
    rows = a.get("variants", {})
    lines = [
        f"# Exp 1 — Core Ablation\n",
        f"**Run dir:** `{run_dir.name}`  ",
        f"**Model:** Gemma-4-31B-it  ",
        f"**Dataset:** CounterFact-1k (W.6 filter)\n",
        "| Variant | n | Mean Margin | Median Margin | Recall@1 |",
        "|---------|---|-------------|---------------|----------|",
    ]
    for v, s in rows.items():
        lines.append(
            f"| {v} | {s.get('n','—')} | {s.get('mean_margin', float('nan')):.4f} "
            f"| {s.get('median_margin', float('nan')):.4f} | {s.get('recall_at_1', 0):.4f} |"
        )
    vf = "✅ PASS" if a.get("verdict_bank_fires") else "❌ FAIL"
    lines += [
        "",
        f"**Verdict (bank_fires):** {vf}",
        "",
        "*Generated by `finalize.py`*",
    ]
    _write_run_readme(run_dir, "exp1_core_ablation", "\n".join(lines) + "\n")


def _exp2_readme(a: dict, run_dir: Path) -> None:
    curves = a.get("curves", {})
    lines = [
        "# Exp 2 — Position Invariance\n",
        f"**Run dir:** `{run_dir.name}`  ",
        "**pre-RoPE degradation:** {:.4f} nats  ".format(a.get("pre_rope_degradation", float("nan"))),
        "**post-RoPE degradation:** {:.4f} nats\n".format(a.get("post_rope_degradation", float("nan"))),
        "| Variant | position_δ | n | Mean Margin |",
        "|---------|-----------|---|-------------|",
    ]
    for variant, curve in curves.items():
        for pt in curve:
            lines.append(
                f"| {variant} | {pt['position_delta']} | {pt['n']} | {pt['mean_margin']:.4f} |"
            )
    vf = "✅ PASS" if a.get("verdict_pre_rope_more_stable") else "❌ FAIL"
    lines += ["", f"**Verdict (pre_rope_more_stable):** {vf}", "", "*Generated by `finalize.py`*"]
    _write_run_readme(run_dir, "exp2_position_invariance", "\n".join(lines) + "\n")


def _exp4_readme(a: dict, run_dir: Path) -> None:
    methods = a.get("variants", {})
    lines = [
        "# Exp 4 — CounterFact Main Result\n",
        f"**Run dir:** `{run_dir.name}`  ",
        "**Model:** Gemma-4-31B-it, 3 seeds, 807 eligible\n",
    ...
    for m, s in methods.items():
        ...
        lines.append(
            f"| {m} | {s.get('n','—')} | {s.get('recall_at_1',0):.4f} "
            f"| {s.get('mean_margin', float('nan')):.4f} "
            f"| [{lo:.4f}, {hi:.4f}] |"
        )
    vf = "✅ PASS" if a.get("verdict_atb_beats_baseline") else "❌ FAIL"
    chi2 = a.get("mcnemar_chi2", float("nan"))
    pval = a.get("mcnemar_p", float("nan"))
    lines += [
        "",
        f"**Verdict (atb_beats_baseline):** {vf}",
        f"**McNemar:** χ²={chi2:.4f}, p={pval:.4f}" if not _isnan(chi2) else "",
        "",
        "*Generated by `finalize.py`*",
    ]
    _write_run_readme(run_dir, "exp4_cf1k_main", "\n".join(lines) + "\n")


def _exp5_readme(a: dict, run_dir: Path) -> None:
    curve = a.get("curve", [])
    lines = [
        "# Exp 5 — α Dense Sweep\n",
        f"**Run dir:** `{run_dir.name}`  ",
        f"**Cliff at α:** {a.get('cliff_alpha', '?')}  ",
        f"**Peak α:** {a.get('peak_alpha', '?')} (margin={a.get('peak_margin', float('nan')):.4f})\n",
        "| α | n | Mean Margin | Recall@1 |",
        "|---|---|-------------|----------|",
    ]
    for pt in curve:
        lines.append(f"| {pt['alpha']} | {pt['n']} | {pt['mean_margin']:.4f} | {pt.get('recall_at_1',0):.4f} |")
    lines += ["", "*Generated by `finalize.py`*"]
    _write_run_readme(run_dir, "exp5_alpha_sweep", "\n".join(lines) + "\n")


def _exp6_readme(a: dict, run_dir: Path) -> None:
    ...
    lines = [
        ...
        f"**Run dir:** `{run_dir.name}`\n",
        "| Variant | n | Mean Margin | Recall@1 |",
        "|---------|---|-------------|----------|",
    ]
    for v, s in variants.items():
        lines.append(
            f"| {v} | {s.get('n','—')} | {s.get('mean_margin', float('nan')):.4f} "
            f"| {s.get('recall_at_1',0):.4f} |"
        )
    lines += ["", "**Verdict:** INVALIDATED — see Exp 6b.", "", "*Generated by `finalize.py`*"]
    _write_run_readme(run_dir, "exp6_negative_controls", "\n".join(lines) + "\n")


def _exp6b_readme(a: dict, run_dir: Path) -> None:
    variants = a.get("variants", {})
    lines = [
    ...
    for v, s in variants.items():
        ...
        lines.append(
            f"| {v} | {s.get('n','—')} | {s.get('mean_margin', float('nan')):.4f} "
            f"| [{lo:.4f}, {hi:.4f}] | {s.get('recall_at_1',0):.4f} |"
        )
    vf = "✅ PASS" if a.get("verdict_correct_dominates") else "❌ FAIL"
    lines += ["", f"**Verdict (correct_dominates):** {vf}", "", "*Generated by `finalize.py`*"]
    _write_run_readme(run_dir, "exp6b_post_rope_negative_controls", "\n".join(lines) + "\n")


def _exp7_readme(a: dict, run_dir: Path) -> None:
    variants = a.get("variants", {})
    bank_size = a.get("bank_size", "?")
    lines = [
        "# Exp 7 — Non-Gemma Pre-RoPE Negative Controls\n",
        f"**Run dir:** `{run_dir.name}`  ",
        f"**bank_key_mode:** pre_rope  bank_size={bank_size}  ",
        "**Model:** Qwen3-4B-Instruct-2507, 807 eligible, seeds 0/1/2\n",
        "| Variant | n | Mean Margin | 95% CI | Recall@1 |",
        "|---------|---|-------------|--------|----------|",
    ]
    for v, s in variants.items():
    ...
    vf = "✅ PASS" if a.get("verdict_correct_dominates") else "❌ FAIL"
    h73 = "✅ PASS" if a.get("h73_k_addressing_pass") else "❌ FAIL"
    h74 = "✅ PASS" if a.get("h74_shuffled_degrades_pass") else "❌ FAIL"
    lines += [
        "",
        f"**H7.1 correct_bank dominates:** {vf}",
        f"**H7.3 K-addressing (random_K_correct_V < correct_bank):** {h73}",
        f"**H7.4 shuffled degrades:** {h74}",
        "",
        "*Generated by `finalize.py`*",
    ]
    _write_run_readme(run_dir, "exp7_non_gemma_pre_rope", "\n".join(lines) + "\n")
...
def main():
    ...
    if run6b:
        ...
        _exp6b_readme(a6b, run6b)
    else:
        print("[exp6b] MISSING")

    # Exp7 — non-Gemma pre-RoPE negative controls
    run7 = latest_run(exp_root / "exp7_non_gemma_pre_rope")
    if run7:
        print(f"[exp7] analyzing {run7}")
        a7 = analyze_exp7(run7)
        all_analyses["exp7"] = a7
        verdicts["V7_pre_rope_qwen3_correct_dominates"] = a7["verdict_correct_dominates"]
        verdicts["V7_k_addressing"] = a7.get("h73_k_addressing_pass", False)
        verdicts["V7_shuffled_degrades"] = a7.get("h74_shuffled_degrades_pass", False)
        _write_neg_controls_tex(a7, out / "paper_tables" / "exp7_non_gemma_pre_rope.tex",
                                caption_tag="(Exp7: pre\\_rope, Qwen3-4B)")
        write_exp7_plot(a7, out / "plots" / "exp7_non_gemma_pre_rope_negative_controls.png")
        _exp7_readme(a7, run7)
    else:
        print("[exp7] MISSING")
...
def _write_readme(analyses: dict, verdicts: dict, out: Path) -> None:
    ...
    # Exp7 table rows
    exp7_rows = ""
    for vname, s in a7.get("variants", {}).items():
        ci = f"[{fmt(s.get('ci_lo'))}, {fmt(s.get('ci_hi'))}]"
        exp7_rows += (
            f"| {vname} | {s.get('n','N/A')} | {fmt(s.get('recall_at_1'))} | "
            f"{fmt(s.get('mean_margin'))} | {ci} | {fmt(s.get('median_margin'))} |\n"
...
| V7 correct bank dominates (pre_rope, Qwen3) | {vc('V7_pre_rope_qwen3_correct_dominates')} |
| V7 K-addressing (pre_rope effective) | {vc('V7_k_addressing')} |
| V7 shuffled degrades (binding matters) | {vc('V7_shuffled_degrades')} |
...
## Exp 7 — Non-Gemma Pre-RoPE Negative Controls (Qwen3-4B-Instruct-2507)

`bank_key_mode=pre_rope`, `bank_size=200` (valid shuffled control).
Model has no native V-norm → pre_rope injection operates at full scale.

| Variant | n | Recall@1 | Mean Margin | 95% CI | Median Margin |
|---------|---|----------|-------------|--------|---------------|
{exp7_rows}

**H7.1 correct_bank dominates:** {vc('V7_pre_rope_qwen3_correct_dominates')}  
**H7.3 K-addressing:** {vc('V7_k_addressing')}  
**H7.4 Shuffled degrades:** {vc('V7_shuffled_degrades')}

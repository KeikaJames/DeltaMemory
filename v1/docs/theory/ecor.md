...
## 第七节：W-T3.6 实施记录与剩余 TODO

### 已完成（W-T3.6 round 1 + round 2）

- [x] **接线**：`LOPIConfig.use_ecor` + `LOPIConfig.ecor_cfg` 已加入；`apply_lopi` 在 ECOR-on 路径下调用 `lopi_inject` 并通过减去 `v_ctx_readout` 恢复 bank 贡献。`use_ecor=False` / `ecor_cfg.enabled=False` / `soft_blend=0` 三条短路保留逐位等价（commit `dfb6e4db`）。
- [x] **单元测试**：`tests/test_lopi_ecor_routing.py` 4 用例（α=0 退化等价、`use_ecor=False` 等价、`soft_blend=0` 等价、`soft_blend=1` 实际改变输出 + asdict 序列化）全部通过。
- [x] **算子级 ablation**（round 1）：`experiments/W_T3_6_ecor_op/COMBINED_REPORT.md`。norm-ratio = 1.0000 在 ortho+soft_blend=1，additive α=4 扰动 V_ctx 17.7×。
- [x] **端到端 NLL A/B**（round 2）：`experiments/W_T3_6_ecor_e2e/COMBINED_REPORT.md`。Qwen2.5-0.5B α=4 ECOR_pure 比 add 减 −1.75 nats；Qwen2.5-1.5B α∈{1,2} ECOR_blend50 最佳；α=0 跨模式 max-abs-logit-diff = 0.000e+00（两模型）。
- [x] **升格决策**：判定为 `mixed`，**默认不翻**。ECOR 保持 opt-in。

### 仍开放

- [ ] **ROME 干扰测试**：`tests/test_ecor_rome_interference.py`（验证同时使用 ECOR 和 ROME writer 时 ROME 事实召回不显著下降）
- [ ] **梯度饱和分析**：`experiments/W_T36_ecor_grad/` 不同 $k$ 值下 $\gamma_t$ 梯度范数曲线
- [ ] **自适应 k 接线**：`lopi_profiler.py` 输出推荐 $k$ 值（基于 $\|M_\perp\| / \|V_{\text{ctx}}\|$ 分布）
- [ ] **跨架构 e2e**：扩展到 Llama-3.2-1B、gemma-3-1b-it（验证 0.5B α=4 处的 additive blow-up 是否在更多小模型重现）

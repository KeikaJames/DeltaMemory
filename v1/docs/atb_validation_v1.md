...
| V9 (Exp31, H_A) | A learned K-adapter (`A: q → k_bank`) is the missing piece; embedding-space discriminability is the bottleneck | ❌ FAIL — InfoNCE training reached ~40× chance embedding val top-1, but LM-output Gate B = 0/375 at every α, and a shuffled-pair Gate-E control adapter beat the correctly-trained one. See [`experiments/atb_validation_v1/exp31_learned_k_adapter/EXP31_VERDICT.md`](../experiments/atb_validation_v1/exp31_learned_k_adapter/EXP31_VERDICT.md). |
| V10 (Exp32, H_B) | Attention is the wrong injection site; MLP-output gated readout (ROME/MEMIT fact-storage locus) restores routing | ❌ FAIL — 3 seeds × n=125 × N=200 × α∈{0.1,0.3,1.0}. Embedding val top-1 = **92%** (~106× chance), but LM-output Gate B = 0/375. Gate D (shuffled_factids) beats learned by **1.17 logits**; Gate E (shuffled-pair router) beats learned by **1.42 logits**. See [`experiments/atb_validation_v1/exp32_mlp_side_gated_memory/EXP32_VERDICT.md`](../experiments/atb_validation_v1/exp32_mlp_side_gated_memory/EXP32_VERDICT.md). |
| **Architectural ceiling** | H_A and H_B are mutually orthogonal; if either succeeds, the cosine-routed ANB framing remains viable | ❌ DOUBLE NEGATIVE — both rejected with matching diagnostic (strong in-isolation routing, zero LM-output Gate B, controls beating signal). The failure mode is the α-scaled residual readout protocol itself, independent of read site or routing capacity. |

"""Exp15 — K/V binding causality (2×2 matrix).
...
def main():
    ...
    if args.n > 0:
        rows = rows[: args.n]
    n_eligible = len(rows)
    print(f"[exp15] eligible rows = {n_eligible} (dropped {dropped} pre-filter)")
    ...
    for seed in seeds:
        ...
        print(f"[exp15] seed={seed} kept {len(kept_ids)}/{n_eligible}", flush=True)
    ...
    patcher.remove()
    dt = time.time() - t0
    print(f"[exp15] done in {dt:.1f}s -> {rows_path}")
